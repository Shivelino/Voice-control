...
class OpenSmileFeatureSet:
    ...
    def feature_extraction(self, config_file, output_file):   # 输出csv文件
        """
        利用openSmile工具特征提取
...
class RhythmFeatures:
    ...
    def activity_detect(self, min_interval=15, e_low_multifactor=1.0, zcr_multifactor=1.0, pt=False):
        ...
        energy_high = energy_average / 3  # 能量均值的4分之一作为能量高阈值 如果有噪音得把除数调低
        energy_low = (sum(ste[:10]) / 10 + energy_high / 5) * e_low_multifactor  # 前5帧能量均值+能量高阈值的5分之一作为能量低阈值 改个前十帧,1/3试试
